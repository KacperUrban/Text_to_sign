...
def measure_response_time(data: pd.DataFrame, device: str, model: AutoModelForSeq2SeqLM, tokenizer: AutoTokenizer, hyperparams: dict):
    train_data = data
    test_data = data.copy()
    ...
    return np.mean(times_list), np.std(times_list), np.mean(length_sentence), np.std(length_sentence)
